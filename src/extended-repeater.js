...
function repeater(str, options) {
  ...
  const repeatTimes =
    options && typeof options.repeatTimes === "number"
      ? options.repeatTimes
      : 1;
  const separator =
    options && typeof options.separator === "string" ? options.separator : "+";
  const addition =
    options && typeof options.addition !== "undefined"
      ? String(options.addition)
      : "";
  const additionRepeatTimes =
    options && typeof options.additionRepeatTimes === "number"
      ? options.additionRepeatTimes
      : 1;
  const additionSeparator =
    options && typeof options.additionSeparator === "string"
